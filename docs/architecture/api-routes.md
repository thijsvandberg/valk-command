...
| Route | Method | Purpose |
|-------|--------|---------|
| `/api/epics/[key]/writer/session` | GET | Get active epic session (phase + messages + drafts + breakdown cards + `relatedCandidates`); resumable. Created cards are enriched with their live sprint (`liveSprintId` from `ticket.sprintName`, `liveSprintName` via `sprintNameCache`; both null for DRAFT cards and backlog). `relatedCandidates` are the persisted find-related results (BRDG-490 #10), so the linkable Related view survives a reload |
...
| `/api/epics/[key]/writer/messages` | POST | Send a chat turn. Phase-aware: `feed` invokes `write-story-draft` (epic body enrichment); `discovery`/`breakdown`/`refine`/`sprints` invoke the `break-down-epic` skill with the current phase + existing breakdown. The `refine` phase (renamed from `detail` in BRDG-488) additionally carries the `<story-detail index="N">` tag contract so deepened cards land back on the board. No direct LLM in Bridge |
| `/api/epics/[key]/writer/messages` | DELETE | Dismiss a single message (`?id=`), or clear the whole conversation with `?all=true` (BRDG-489), keeping the session, epic draft, and breakdown cards. Mirrors the ticket story-writer messages DELETE |
...
| `/api/epics/[key]/writer/cards/[index]` | PATCH | Edit a DRAFT child card in place (BRDG-490 #5). Partial body: any of `{ title?: string, bullets?: string[], body?: string \| null }` (at least one; else 400). Title must be non-empty; bullets are trimmed and blanks dropped; an empty/whitespace body clears the detail (depth -> bullets). Only provided fields are written |
| `/api/epics/[key]/writer/cards/reorder` | PUT | Persist a manual drag-reorder of the breakdown cards (BRDG-487). Body: `{ orderedIds }` (card ids in their new top-to-bottom order; must be a permutation of the current cards, else 400). Reassigns `card_index` to 0..N-1 and remaps every `suggestedLinks.targetIndex` through the old->new index table in one transaction (a two-phase temp-offset write avoids the `(session_id, card_index)` unique index tripping mid-reorder) -> `{ ok, orderedIds }` |
...
| `/api/tickets/[key]/story-writer/messages` | DELETE | Dismiss a single pending/failed message (`?id=<messageId>`), or clear the whole conversation with `?all=true` (BRDG-489). A clear keeps the session, its draft/local-edits, and breakdown cards; AI drafts survive (their `message_id` FK is `ON DELETE SET NULL`). Resolves the active session by key, so it serves both story and epic sessions |
