...
Key bottlenecks / lessons:
- **The avatar gate ate ~an hour on a self-inflicted red herring, and the ACTUAL cause was a unit error.** The avatar (`@[44rem]`) read `display:none` at what I *thought* was a 45.94rem row while the 40rem gate showed, so I chased phantoms: block-vs-flex cascade, a Tailwind "class abutting `${` in a template literal isn't scanned" theory, dev CSS staleness, `rm -rf .next`, and even a content-box-vs-border-box theory. **The real cause: this app's root font-size is 17px (see [[project_boardrow_accent_border_offset]]), so a `rem`-based container query triggers at `N × 17px`, but I converted the measured px width to rem with `/16`.** The row was 735px = 43.2rem (÷17), genuinely below the 44rem (748px) gate; my `/16` math reported it as 45.94rem and made the gate look broken. `@[48rem]` = 816px is the shipped value. Lesson: on this codebase convert px↔rem with `/17`, and when a container-query gate seems off by a hair, check the rem math FIRST — before the build pipeline.
- **CSSOM introspection is unreliable in Turbopack dev.** `document.styleSheets` walks kept returning empty/partial rule sets (even `.hidden` came back missing), and the compiled syntax is `@container boardrow (width >= Nrem)`, not `min-width:` — so both my grep and my CSSOM checks initially misfired. Verify container-query behaviour by computed `display` on a live element, not by reading rules. (Probing the bare class inside a real `@container/boardrow` row is the reliable check.)
...
| Phase | Notes |
|---|---|
| Plan (Opus) | High value. Corrected the story's facts before any code: 7 missing exports not 5 (`_requestTimestamps`/`_noteRateLimitApproaching` omitted), and `SPRINT_FIELD`-family ARE real re-exports — so the guard had to be a dynamic `import * as real` superset, not a hand-typed list. Also prescribed the safe ORDER (touch the runner plumbing LAST) to avoid deadlocking the session's own hook, and flagged `isLive` getter-vs-writable as the migration's one real risk. |
| Implement | The universal transform turned out trivial and low-risk: wrapping each inline `() => ({...})` in `createJiraClientMock({...})` makes every inline value a winning override while the factory only ADDS previously-undefined exports. A codemod (run via Bash, not Edit) did all 52 without firing the hook 100+ times. |
| Verify | Full suite + build green; non-UI story so no browser pass. |

Key bottlenecks / lessons:
- **vitest 4 dropped `poolOptions.forks.maxForks` from the typed config and `minWorkers` isn't a key.** `maxWorkers: <n>` is the only top-level worker cap that typechecks. Cost two tsc cycles. Grep `node_modules/vitest/dist/chunks/*.d.ts` for the real option names rather than assuming v3 shapes.
- **`get isLive()` overrides broke under the factory's spread.** Several files drive `isLive` via a top-level test variable getter; inline that stayed lazy, but `{ ...overrides }` evaluates getters eagerly at vi.mock-hoist time → `Cannot access 'mockIsLive' before initialization` (4 files). Fix was general, not per-file: merge `jiraClient` overrides with `Object.defineProperties(jc, Object.getOwnPropertyDescriptors(overrides))` so getters stay lazy. Spreading override objects that may contain getters is a hoisting trap.
- **`timeout` is not on macOS.** Three smoke-test attempts silently no-op'd (`command not found: timeout` killed the pipe before vitest ran), masquerading as "vitest found nothing". On darwin use `gtimeout` or no timeout.
- **Bracketed test paths (`[key]`/`[id]`) need `set -f` + single-quoted literals.** zsh globs `[key]` to nothing; escaping (`\[key\]`) makes vitest treat it as a regex char-class. vitest does *substring* matching, so `set -f; npx vitest run 'src/.../[key]/route.test.ts'` is the reliable form. Same family as the BRDG-232 bracket-glob lesson; `GIT_LITERAL_PATHSPECS=1` is the git equivalent for staging them.
- **`vitest run --changed` runs the FULL suite when a config/`package.json` file is in the diff** (it can't scope a non-source change), so the PostToolUse hook will occasionally run all 645 files (capped by its 120s timeout + skip-on-lock). For ordinary single source/test edits it scopes to the dependents — the win that motivated moving the hook off the full suite.
- **Touching the harness while running under it is safe if sequenced.** Config + factory + 52 migrations + guards all landed while `npm run test` was still plain `vitest run` (the old in-memory hook behaved normally); the wrapper/`package.json`/`settings.json` rewire went last, and `.sh`/`.json`/`package.json` edits don't match the hook's `.ts` matcher, so the final swap fired nothing. A distinct lock name (`/tmp/valk-vitest.lock` vs the old hook's `/tmp/valk-test.lock`) also defused this-session reentrancy.
...
## BRDG-490 / BRDG-491 — Epic Writer misc improvements rounds 3 & 4 (2026-07-06/07)

Two batches of Epic Writer polish (490: 10 items incl. editable draft cards, markdown detail, related-stories panel, header phase rail; 491: 3 follow-ups from PO feedback — shared send/stage split model, compact phase control, `/clear` command autocomplete). Both landed green (8027 tests, build). One operational snag worth recording.

Key bottleneck / lesson:
- **Running the full `vitest` suite while the dev server is up thrashed a 16GB machine.** The BRDG-491 final full-suite run took ~40 min (vs the usual ~180s) and reported "1 failed | 16 errors" — but those were worker OOM casualties (only 684/700 files ran), not real regressions: every changed file passed in isolation. Killing the dev server (`lsof -ti:3101 | xargs kill -9`) before re-running gave a clean 178s / 8027-passed run. Lesson: the dev server competes with vitest for memory even though it isn't a "test process"; stop it (or don't restart it) before the final full-suite gate. CLAUDE.md already warns about concurrent vitest; this extends it to the dev server.
- **The focusless-button ratchet (BRDG-421) is a source-text scanner, not a render check.** A multi-line `onClick={() =>` with the arrow at line-end truncated the scanned opening tag before `className`, so a genuinely focus-bearing button read as focusless (8 > 7). Fix was formatting (brace-body `onClick={() => { ... }}`), not a real a11y miss. Lesson: keep `className` scannable — avoid `=>\n` at the end of an attribute line on a raw `<button>`.
