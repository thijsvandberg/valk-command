...
- [ ] A muted callout/banner is shown above the ticket columns for active sprints only
- [ ] The summary sentence is computed from sprint data using the following rules (evaluated in order):
...
- [ ] Working days remaining are calculated from the sprint end date, excluding weekends
- [ ] The banner is not shown for completed or future sprints
- [ ] The summary updates reactively when sprint data changes (e.g. after a refresh)
