...
- [ ] Tickets in the ticket list (`TicketRow`) show a small conflict badge/icon when `editState === "conflict"`
- [ ] Tickets in the ticket list show a subtle "local edits" indicator when `editState === "local_edits"` or `editState === "draft"`
- [ ] Tickets in the queue panel (`SortableQueueItem`) show the same conflict/local-edits indicators
- [ ] If any ticket in the current queue has a conflict, the queue panel header shows a warning (e.g. "2 conflicts")
- [ ] Conflict badge uses a warning color (amber/orange); local-edits badge uses a neutral informational color
- [ ] Hovering/focusing the badge shows a tooltip explaining the state ("Jira version changed since last local edit" / "Has unsaved local changes")

### Active sync check

- [ ] When the refinement page mounts, all tickets belonging to the active session are checked for fresh edit state (batch API call or individual re-validation)
- [ ] When a ticket is added to the queue, its edit state is re-fetched so the indicator is up-to-date
- [ ] The check does not block the page render; indicators update asynchronously once results arrive
- [ ] A subtle loading state (e.g. shimmer on the badge area) shows while the check is in progress
- [ ] Re-check can be triggered manually via a refresh action on the queue panel

### Edge cases

- [ ] Tickets without any local edits (`editState === "clean"`) show no indicator
- [ ] If the batch check fails (network error), existing cached state is preserved and no false positives are shown
- [ ] Tests cover: conflict badge rendering, local-edits badge rendering, clean state (no badge), async check on mount, check on ticket add
