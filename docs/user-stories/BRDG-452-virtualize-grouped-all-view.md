...
- [ ] In the All view with a large board (hundreds+ of tickets across many sprints), only the
      rows near the viewport are in the DOM; scrolling stays smooth. <!-- TicketTable groupedTable windowed via useVirtualizer over a flattened item list -->
- [ ] Group headers (`GroupStatBar`), collapse/expand, per-group status filter, sync, create,
      the "Finished work" divider, and placeholders all still render and behave as before. <!-- flattened item types render the same components -->
- [ ] Drag & drop in the All view is smooth end-to-end (start AND drag-through): reorder within
      a sprint, cross-sprint drop onto a row and onto an empty group-zone, and drop onto a
      sprint-slot tile all still work. <!-- useSprintBoardDragDrop.ts handlers unchanged; SprintBoard dndMeasuringStrategy revisited -->
- [ ] The virtual window is correctly positioned under content above the table (analytics
      panel), as in the flat view. <!-- reuse BRDG-416 scrollMargin layout-effect -->
- [ ] No regression in the BRDG-405/416 per-row render fan-out (toggling one row does not
      re-render the others) or in the pending-edits / pending-moves overlay. <!-- TicketTable.renderCount.test.tsx stays green -->

## Tests

- [ ] Grouped-virtualization test: with many groups/rows, assert only a windowed subset of
      `BoardRow`s mount (mirror the flat virtualizer test / TicketTable.renderCount harness). <!-- src/components/sprint-board/TicketTable.*.test.tsx -->
- [ ] Grouped DnD test stays green (cross-sprint drop, group-zone, reorder). <!-- SprintBoardDragDrop.test.tsx -->
- [ ] Existing SprintBoard / TicketTable / render-count / moveMeter tests stay green.
