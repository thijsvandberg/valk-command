...
NOTE (verification gotcha): this app's **root font-size is 17px**, not 16px (see [[project_boardrow_accent_border_offset]]). A `rem`-based container-query gate therefore triggers at `N × 17px` (e.g. `@[48rem]` = 816px, `@[26rem]` = 442px). When calibrating against a measured pixel width, convert px->rem with `/17`; using `/16` overstates the width ~6% and makes a gate look like it fires at the wrong point. Verify by computed `display` on a live row, not by eyeballing rem from `getBoundingClientRect`.
