-- Add 'story-writer' to activity_log type CHECK constraint (SQLite requires table rebuild)
PRAGMA foreign_keys=OFF;--> statement-breakpoint
CREATE TABLE `activity_log_new` (
	`id` text PRIMARY KEY NOT NULL,
	`type` text NOT NULL CHECK(type IN ('sprint-sync', 'ticket-sync', 'single-ticket', 'comment-sync', 'webhook', 'review', 'metadata-update', 'local-edit', 'push-to-jira', 'bulk-action', 'story-writer')),
	`scope` text,
