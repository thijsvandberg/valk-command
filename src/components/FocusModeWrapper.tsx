...
function FocusModeLayout({ children }: { children: ReactNode }) {
  ...
  return (
    <div className="flex flex-col h-screen bg-surface-base text-text-primary">
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-notification focus:rounded-lg focus:bg-[var(--color-brand-600)] focus:px-4 focus:py-2 focus:text-white focus:text-body-lg focus:shadow-lg focus:outline-none focus-visible:ring-1 focus-visible:ring-inset focus-visible:ring-[var(--color-brand-500)]/50"
      >
        Skip to content
      </a>
      ...
      <button
        ...
        style={style}
        className={`fixed ${CORNER_CLASSES[corner]} z-dropdown flex h-8 w-8 items-center justify-center rounded-lg cursor-pointer touch-none
