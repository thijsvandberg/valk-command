...
export function FilterChip({ active = false, onClick, children }: FilterChipProps) {
  return (
    <button
      ...
      onClick={onClick}
      aria-pressed={active}
      className={`cursor-pointer rounded-md border px-2.5 py-1 text-label font-[var(--font-body)] transition-[color,background-color,border-color,transform] duration-150 focus-visible:outline-2 focus-visible:outline-offset-1 focus-visible:outline-[var(--color-brand-400)] active:scale-95 ${
        active
