...
import { useSyncContext, type SyncState } from "@/contexts/SyncContext";
import { useSyncStatus } from "@/hooks/useSprintBoard";
import type { SyncLogEntry } from "@/types/ticket";

function stateIcon(state: SyncState, errorCount: number) {
  if (state === "syncing") {
    return (
      <RefreshCw
...
function statusDot(status: SyncLogEntry["status"]) {
...
export function SyncIndicator({ collapsed }: { collapsed: boolean }) {
  const { syncState, lastSync, unacknowledgedErrors, runningEntries, cancelSync, cancelAllSyncs } = useSyncContext();
  const { data: recentEntries } = useSyncStatus(8);
  ...
  return (
    <div className="relative" ref={panelRef}>
      ...
      <button
        ...
        title={collapsed ? stateLabel(syncState) : undefined}
      >
        <span className="relative shrink-0">
          {stateIcon(syncState, errorCount)}
          {errorCount > 0 && (
        ...
        {!collapsed && (
          <>
            <span className="flex-1 text-left text-xs font-[var(--font-body)] truncate">
              ...
              {lastSync?.completedAt && syncState === "idle" && (
                <span className="text-white/20 ml-1.5">{timeAgo(lastSync.completedAt)}</span>
              )}
            </span>
            <ChevronUp
        ...
        >
          <div className="px-3 py-2.5 border-b border-white/[0.06] flex items-center justify-between">
            ...
            {runningEntries.length > 0 && (
              <button
                type="button"
                onClick={(e) => { e.stopPropagation(); cancelAllSyncs(); }}
              ...
              >
                ...
                <div className="flex-1 min-w-0">
                  ...
                  {entry.status === "running" && (
                    <button
                      type="button"
                      onClick={(e) => { e.stopPropagation(); cancelSync(entry.id); }}
