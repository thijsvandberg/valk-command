...
describe("usePageTitle", () => {
  it("sets document.title with the Bridge suffix", () => {
    renderHook(() => usePageTitle("Dashboard"));

    expect(document.title).toBe("Dashboard | Bridge");
  });
  ...
  it("returns a <title> element with the full title", () => {
    ...
    expect(el.props.children).toBe("Sprint Board | Bridge");
  });
  ...
  it("updates document.title when the title prop changes", () => {
    ...
    expect(document.title).toBe("Chat | Bridge");

    rerender({ title: "Settings" });

    expect(document.title).toBe("Settings | Bridge");
  });
  ...
  it("handles an empty string title", () => {
    ...
    const el = result.current as React.ReactElement<{ children: string }>;
    expect(el.props.children).toBe(" | Bridge");
  });
});
