...
export const EMPTY_DEV_INFO: DevInfoPayload = { branches: [], pullRequests: [], commits: [], builds: [], deployments: [] };
...
export function normalisePrStatus(raw: string): DevPullRequest["status"] {
...
export function normaliseBuildState(raw: string): DevBuild["state"] {
  const upper = raw.toUpperCase();
...
/**
 * Fetches all development info (branches, PRs, commits, builds, deployments)
 * for a given ticket key from Bitbucket Cloud.
 */
export async function fetchDevInfo(key: string): Promise<DevInfoPayload> {
  if (!isConfigured()) return EMPTY_DEV_INFO;
  ...
  try {
    ...
    return { branches, pullRequests, commits, builds, deployments };
  } catch (err) {
    logger.error("dev-info", "fetch failed:", err);
    return EMPTY_DEV_INFO;
  }
}
